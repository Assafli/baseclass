...
import (
	...
	"github.com/ignite/cli/baseclass/pkg/nodetime"
	"github.com/ignite/cli/baseclass/pkg/protoc"
)
...
func NewToolsIBCSetup() *cobra.Command {
	return &cobra.Command{
		...
		Example: `baseclass tools ibc-setup -- -h
baseclass tools ibc-setup -- init --src relayer_test_1 --dest relayer_test_2`,
	}
}
...
func NewToolsIBCRelayer() *cobra.Command {
	return &cobra.Command{
		...
		Example: `baseclass tools ibc-relayer -- -h`,
	}
}
...
func NewToolsProtoc() *cobra.Command {
	return &cobra.Command{
		...
		Example: `baseclass tools protoc -- --version`,
	}
}
...
Bash:
  ...
  $ source <(baseclass  tools completions bash)

  To load completions for every new session, run:

  ** Linux **
  $ baseclass  tools completions bash > /etc/bash_completion.d/baseclass

  ** macOS **
  $ baseclass  tools completions bash > /usr/local/etc/bash_completion.d/baseclass
...
Zsh:
  ...
  $ baseclass  tools completions zsh > "${fpath[1]}/_ignite"

  You will need to start a new shell for this setup to take effect.

fish:

  $ baseclass  tools completions fish | source

  To load completions for each session, execute once:
  
  $ baseclass  tools completions fish > ~/.config/fish/completionss/baseclass.fish

PowerShell:

  PS> baseclass  tools completions powershell | Out-String | Invoke-Expression

  To load completions for every new session, run:
  
  PS> baseclass  tools completions powershell > baseclass.ps1
